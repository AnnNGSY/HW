# tested on 5.22.1, both 64 bits Windows 8 and 32 bits Windows 8.1
package Player;
{
	use strict;
	use warnings;
...
package Human;
{
	...
	sub nextMove # assumes input always 2 integers with space in middle
	{
		...
		# get input form standin split with space and remove return key
		chomp(my @position = split(/ /, <STDIN>)); 
		return @position;
	}
}
...
package Computer;
{
	...
	sub nextMove
	{
		...
		# version 1: choose a valid position randomly.(Easier, but you get full mark 95 only)< finished.
		# version 2: choose a position which minimizes the opponent’s mobility 
		# The mobility of the opponent is defined as the number of valid moves
		# allowed to the opponent after the player puts a piece to a position
	}
}
...
package Reversi;{ # models the Reversi game
	...
	sub startGame{		
		...
		while (1) {	# Perl : non-zero => false
			...
			# Obtain a position, put disc and filp if vaild
			if(($self->putDisc(@move = ($self->{playerList}[$self->{turn}]->nextMove($self)), 1))){
	...
	sub putDisc{
		...
		# when flag = 1, put disc and return move vaild or not
		# when flag = 0, return move vaild or not? return validmoves of oppent after you make this move?
		my $ret = 0;
